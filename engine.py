...
def _div_in_sweep_context(div: dict, sweep: "_Sweep", n: int) -> bool:
    """
    Verifica si alguno de los 4 modelos de divergencia ocurrió
    dentro del contexto del sweep (entre se_bar y next_se).
    La señal de divergencia se marca en la segunda barra del par.
    """
    se_i = sweep.se_bar
    # Ventana: desde SE hasta 200 barras después (o fin de datos)
    ...
    for key in ["m1_bull", "m1_bear", "m2_bull", "m2_bear",
        ...
        for j in range(se_i, window_end):
            if series[j] == 1.0:
                return True, j  # divergencia encontrada en barra j
    return False, -1
...
def simulate(data: pd.DataFrame, risk: dict, entry_confs: list, exit_confs: list) -> dict:
    ...
    # Índice de qué sweeps ya hemos procesado
    sweep_idx = 0
    ...
    for i in range(1, n):
        ...
        if not in_trade:
            ...
            for sw_i in range(sweep_idx, len(sweeps)):
                ...
                div_found, div_bar = _div_in_sweep_context(div, sw, n)

                if not div_found or div_bar > i:
                    # Si la divergencia está en el futuro, no procesar aún
                    if not div_found:
                        sweep_idx = sw_i + 1  # sweep sin divergencia, descartar
                    continue
                ...
                entry_dir     = direction
                in_trade      = True
                sweep_idx     = sw_i + 1  # avanzar al siguiente sweep
                break
