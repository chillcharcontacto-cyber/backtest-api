...
def simulate(data: pd.DataFrame, risk: dict, entry_confs: list, exit_confs: list) -> dict:
    ...
    # Detectar si el usuario usa cut_early como condición de salida
    has_cut_early = any(c.get("indicator", c.get("name", "")) == "cut_early"
                        for c in exit_confs) if exit_confs else False
    ...
    for i in range(1, len(prices)):
        ...
        if not in_trade:
            ...
            if entry_signal:
                ...
                # Pre-calcular SL para cut_early (BB/AA del BOS)
                if has_cut_early and _CUT_EARLY_AVAILABLE:
                    try:
                        bos_levels = _get_bos_levels(data)
                        sl_bull    = bos_levels["sl_bull"]
                        sl_bear    = bos_levels["sl_bear"]
                        # Determinar dirección por qué SL está disponible
                        if not np.isnan(sl_bull[i]):
                            entry_sl = float(sl_bull[i])
                        elif not np.isnan(sl_bear[i]):
                            entry_sl = float(sl_bear[i])
                        else:
                            entry_sl = float(data["Low"].values[i-1]) if i > 0 else 0.0
                    except Exception:
                        entry_sl = float(data["Low"].values[i-1]) if i > 0 else 0.0

        # Evaluar TODAS las confirmaciones de salida (AND)
        elif in_trade:
            # Para cut_early: evaluar con params dinámicos del trade activo
            if has_cut_early and _CUT_EARLY_AVAILABLE:
                direction = 'bull' if entry_sl < entry_price else 'bear'
                cut_params = {
                    'direction':   direction,
            ...
            # Salida normal por confirmaciones precomputadas (skip dynamic)
            normal_exit = all(
                check_condition(c["series"], c["condition"], c["value"], close, i)
                for c in exit_computed
                if not c.get("dynamic", False)
            )

            exit_signal = normal_exit or cut_active
            ...
            if exit_signal:
                ...
                trades.append({
                    ...
                    "exit_type":  "cut_early" if cut_active else "normal",
                })
                in_trade      = False
                shares        = 0.0
                entry_bar_idx = -1
