...
def compute_sl_filter(data: pd.DataFrame, min_pips: float, max_pips: float,
    ...
    """
    Filtra trades por tamaño de SL.
    SL aproximado = distancia desde Close hasta High/Low de barra anterior.
    ⚠️ PENDIENTE: actualizar cuando BOS esté implementado.
    """
    ...
    for i in range(1, n):
        # Distancia al high anterior (ventas) y low anterior (compras)
        sl_buy  = abs(close[i] - low[i-1])   / pip_size
        sl_sell = abs(close[i] - high[i-1])  / pip_size
        sl_pips = min(sl_buy, sl_sell)  # el menor de los dos
...
def compute_ol_levels(data: pd.DataFrame, liq_strength: int = 25) -> dict:
    """
    Calcula el Opposite Liquidity (OL) para cada barra.
    - OL compras = punto más alto entre SS y SE (ssl sweep window)
    - OL ventas  = punto más bajo entre SS y SE (bsl sweep window)
    ⚠️ PENDIENTE: SL se actualizará con BOS real.
    """
    liq = compute_liquidity(data, strength=liq_strength)
    ...
    ssl_taken = liq["ssl_taken"].values
    bsl_taken = liq["bsl_taken"].values

    ssl_positions = [i for i in range(n) if ssl_taken[i] == 1.0]
    bsl_positions = [i for i in range(n) if bsl_taken[i] == 1.0]

    # OL compras: max close entre SS y SE de ssl_taken
    for idx, ss_i in enumerate(ssl_positions):
        next_ss = ssl_positions[idx + 1] if idx + 1 < len(ssl_positions) else n
        # SE: primera barra donde close supera el nivel barrido
        for j in range(ss_i + 1, next_ss):
            if close_v[j] > close_v[ss_i]:
                se_i = j
                segment = high_v[ss_i:se_i + 1]
                ol_price = segment.max()
                # Asignar OL a todas las barras después de SE hasta el próximo SS
                for k in range(se_i, next_ss):
                    ol_bull[k] = ol_price
                break

    # OL ventas: min close entre SS y SE de bsl_taken
    for idx, ss_i in enumerate(bsl_positions):
        next_ss = bsl_positions[idx + 1] if idx + 1 < len(bsl_positions) else n
        for j in range(ss_i + 1, next_ss):
            if close_v[j] < close_v[ss_i]:
                se_i = j
                segment = low_v[ss_i:se_i + 1]
                ol_price = segment.min()
                for k in range(se_i, next_ss):
                    ol_bear[k] = ol_price
                break
...
def compute_rr_mct_filter(data: pd.DataFrame, min_rr: float,
    ...
    SL = high/low de barra anterior (⚠️ PENDIENTE: actualizar con BOS).
    ...
    for i in range(1, n):
        ...
        # Intento compra: OL bull disponible
        if not np.isnan(ol_bull[i]):
            sl_dist = abs(price - low_v[i-1])
            tp_dist = abs(ol_bull[i] - price)
            if sl_dist > 0 and tp_dist / sl_dist >= min_rr:
                result[i] = 1.0

        # Intento venta: OL bear disponible
        if not np.isnan(ol_bear[i]) and result[i] == 0:
            sl_dist = abs(price - high_v[i-1])
...
def compute_mct_exit(data: pd.DataFrame, liq_strength: int = 25,
    ...
    - Si RR entrada→OL >= rr_threshold (default 3): TP en Fib 0.71
      - Compras:  Fib desde OL hasta BB → TP = OL - 0.71 * (OL - BB)
      - Ventas:   Fib desde OL hasta AA → TP = OL + 0.71 * (AA - OL)
    Devuelve 1.0 cuando el precio alcanza el TP calculado.
    ⚠️ PENDIENTE: SL para RR se actualizará con BOS real.
    """
    close_v = data["Close"].values
    high_v  = data["High"].values
    low_v   = data["Low"].values
    ...
    for i in range(1, n):
        ...
        # ── Salida compras ──
        if not np.isnan(ol_bull[i]):
            sl_dist = abs(price - low_v[i-1]) if i > 0 else 0
            tp_dist = abs(ol_bull[i] - price)

            if sl_dist > 0 and tp_dist / sl_dist >= rr_threshold:
                # BB = mínimo de close en la ventana actual
                window_start = max(0, i - 50)
                bb = low_v[window_start:i].min()
        ...
        # ── Salida ventas ──
        elif not np.isnan(ol_bear[i]):
            sl_dist = abs(price - high_v[i-1]) if i > 0 else 0
            tp_dist = abs(price - ol_bear[i])

            if sl_dist > 0 and tp_dist / sl_dist >= rr_threshold:
                # AA = máximo de close en la ventana actual
                window_start = max(0, i - 50)
                aa = high_v[window_start:i].max()
...
INDICATOR_CATALOG = {
    ...
    "sl_filter": {
        ...
        "conditions":  ["is_true"],
        "needs_value": False,
        "note":        "⚠️ PENDIENTE actualizar con BOS real.",
    },
    "rr_mct": {
        ...
        "conditions":  ["is_true"],
        "needs_value": False,
        "note":        "⚠️ PENDIENTE actualizar SL con BOS real.",
    },
    "mct_exit": {
        ...
        "conditions":  ["is_true"],
        "needs_value": False,
        "note":        "TP = OL normal. Si RR >= threshold: TP en Fib 0.71. ⚠️ PENDIENTE BOS.",
    },
}
