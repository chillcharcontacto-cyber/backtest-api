...
# Cut Early module
try:
    from cut_early import compute_cut_early as _compute_cut_early_ext
    _CUT_EARLY_AVAILABLE = True
except ImportError:
    _CUT_EARLY_AVAILABLE = False
...
def compute_liquidity(data: pd.DataFrame, strength: int = 25,
    ...
    Bug fix: taken_ph_price/taken_pl_price guardan el precio EXACTO
    del pivot cruzado, no el nivel más cercano al precio actual.
    """
    df = data.copy()
    ...
    ph_price_out     = np.full(n, np.nan)   # nivel más cercano (referencia visual)
    pl_price_out     = np.full(n, np.nan)
    taken_ph_price   = np.full(n, np.nan)   # precio EXACTO del pivot cruzado
    taken_ph_bar     = np.full(n, np.nan)
    taken_pl_price   = np.full(n, np.nan)
    taken_pl_bar     = np.full(n, np.nan)
    ...
    for i in range(n):
        ...
        for lvl in active_ph:
            if not lvl.taken:
                if high[i] > lvl.price:
                    ...
                    lvl.taken_at = i
                    ph_taken[i]       = 1.0
                    taken_ph_price[i] = lvl.price      # precio EXACTO del pivot
                    taken_ph_bar[i]   = lvl.bar_index
        ...
        for lvl in active_pl:
            if not lvl.taken:
                if low[i] < lvl.price:
                    ...
                    lvl.taken_at = i
                    pl_taken[i]       = 1.0
                    taken_pl_price[i] = lvl.price      # precio EXACTO del pivot
                    taken_pl_bar[i]   = lvl.bar_index
                elif (i - lvl.bar_index) >= del_after:
                    continue
            surviving_pl.append(lvl)
        active_pl = surviving_pl

        # Nivel más cercano (referencia visual, no usado en detect_sweeps_internal)
        ...
        if untaken_ph:
            ...
            ph_price_out[i] = closest.price
            ph_bar_out[i]   = closest.bar_index
        if untaken_pl:
            closest = min(untaken_pl, key=lambda l: abs(l.price - low[i]))
            pl_price_out[i] = closest.price
            pl_bar_out[i]   = closest.bar_index
    ...
    return {
        ...
        "taken_ph_price": pd.Series(taken_ph_price, index=df.index),
        "taken_ph_bar":   pd.Series(taken_ph_bar,   index=df.index),
        "taken_pl_price": pd.Series(taken_pl_price, index=df.index),
        "taken_pl_bar":   pd.Series(taken_pl_bar,   index=df.index),
    }


def detect_sweeps_internal(data: pd.DataFrame, liq: dict) -> list:
    """
    Construye lista de _Sweep usando el precio EXACTO del pivot cruzado
    (taken_ph_price / taken_pl_price) — bug fix respecto a versión anterior.
    """
    df = data.copy()
    df.columns = [c.capitalize() for c in df.columns]
    high = df["High"].values
    ...
    taken_ph_price = liq["taken_ph_price"].values
    taken_ph_bar   = liq["taken_ph_bar"].values
    taken_pl_price = liq["taken_pl_price"].values
    taken_pl_bar   = liq["taken_pl_bar"].values

    sweeps = []
    for se in range(len(df)):
        # Bearish: pivot high cruzado — sweep_level = precio EXACTO del pivot
        if ph_taken[se] == 1.0 and not np.isnan(taken_ph_price[se]):
            ss          = int(taken_ph_bar[se])
            sweep_level = taken_ph_price[se]
            rng_low     = low[ss: se + 1]
            ol_bar      = ss + int(np.argmin(rng_low))
            ol_price    = rng_low.min()
            sweeps.append(_Sweep('bearish', ss, se, sweep_level, ol_price, ol_bar))

        # Bullish: pivot low cruzado — sweep_level = precio EXACTO del pivot
        if pl_taken[se] == 1.0 and not np.isnan(taken_pl_price[se]):
            ss          = int(taken_pl_bar[se])
            sweep_level = taken_pl_price[se]
...
def evaluate_indicator(
    ...
    if name == "rsi":
        ...
        if not _CUT_EARLY_AVAILABLE:
            raise ValueError("cut_early.py no encontrado en el directorio.")
        # cut_early necesita entry_bar y entry_price del trade activo.
        # En backtest estos se pasan como params dinámicos desde simulate().
        # Como indicador de salida standalone usa defaults seguros.
        return _compute_cut_early_ext(data, params)
...
# Indicadores que NO se precomputan — se evalúan dinámicamente en simulate()
_DYNAMIC_INDICATORS = {"cut_early"}
...
def precompute_indicators(data: pd.DataFrame, confirmations: list) -> list:
    ...
    Indicadores dinámicos (cut_early) se omiten aquí y se evalúan
    barra a barra en simulate() con los params del trade activo.
    """
    computed = []
    for conf in confirmations:
        ...
        if ind_name in _DYNAMIC_INDICATORS:
            # Marcar como dinámico — simulate() lo manejará
            computed.append({
                "name":      ind_name,
                "series":    None,   # sin precompute
                "condition": condition,
                "value":     value,
                "dynamic":   True,
            })
...
def _precompute_sequential(data: pd.DataFrame, liq_strength: int = 25) -> dict:
    """
    Pre-calcula las series de contexto secuencial MCT:
      sweeps      — lista de _Sweep ordenados cronológicamente
      div_signals — dict con las 8 series de divergencia RSI
      bos_signal  — serie con señales BOS (1.0=bull, -1.0=bear, 0=nada)
      bos_levels  — dict con sl_bull, sl_bear, ol_bull, ol_bear
    """
    df = data.copy()
    df.columns = [c.capitalize() for c in df.columns]
    if 'Open' not in df.columns:
    ...
    div = compute_rsi_divergence(df, rsi_period=14,
                                  pivot_strength=5,
                                  liq_strength=liq_strength)

    bos_sig = pd.Series(np.zeros(len(df), dtype=float), index=df.index)
    if _BOS_AVAILABLE:
        try:
            raw_bos = evaluate_bos_indicator(df, {"liq_strength": liq_strength})
            bos_sig = raw_bos
        except Exception:
            pass

    bos_lvls = _get_bos_levels(df, liq_strength)
...
def _div_in_sweep_context(div: dict, sweep: "_Sweep", n: int):
    """
    Verifica si alguno de los 4 modelos de divergencia ocurrió
    dentro del contexto del sweep (entre se_bar y next_se).
    Devuelve (found, bar_index, model_name).
    """
    MODEL_LABELS = {
        "m1_bull": "Model 1 — Regular Bullish",
        "m1_bear": "Model 1 — Regular Bearish",
        "m2_bull": "Model 2 — Sweep Bullish",
        "m2_bear": "Model 2 — Sweep Bearish",
        "m3_bull": "Model 3 — Multiple Bullish",
        "m3_bear": "Model 3 — Multiple Bearish",
        "m4_bull": "Model 4 — Extended Bullish",
        "m4_bear": "Model 4 — Extended Bearish",
    }
    se_i = sweep.se_bar
    window_end = min(se_i + 200, n)

    for key in ["m1_bull", "m1_bear", "m2_bull", "m2_bear",
                "m3_bull", "m3_bear", "m4_bull", "m4_bear"]:
        series = div[key].values
        for j in range(se_i, window_end):
            if series[j] == 1.0:
                return True, j, MODEL_LABELS[key]
    return False, -1, ""


def simulate(data: pd.DataFrame, risk: dict, entry_confs: list, exit_confs: list) -> dict:
    """
    Simulación secuencial MCT:
      1. Sweep activo (SS → SE abre contexto)
      2. Divergencia RSI dentro del contexto del sweep
      3. BOS después de la divergencia
      4. Validación: sesión horaria + SL dentro del rango
      5. Entrada al cierre de la vela del BOS
      6. Gestión: Cut Early + MCT Exit
    """
    close    = data["Close"]
    ...
    # ── Detectar liq_strength de los params del usuario ──
    liq_strength = 25
    for c in entry_confs:
        if c.get("indicator") in ("liquidity", "bos", "rr_mct", "sl_filter"):
            p = c.get("params", {})
            if "liq_strength" in p or "strength" in p:
                liq_strength = int(p.get("liq_strength", p.get("strength", 25)))
                break

    # ── Pre-computar contexto secuencial ──
    seq = _precompute_sequential(data, liq_strength)
    sweeps     = seq["sweeps"]
    div        = seq["div"]
    bos_signal = seq["bos_signal"].values
    bos_levels = seq["bos_levels"]
    sl_bull    = bos_levels["sl_bull"]
    sl_bear    = bos_levels["sl_bear"]
    ol_bull    = bos_levels["ol_bull"]
    ol_bear    = bos_levels["ol_bear"]
    ...
    for i in range(1, n):
        ...
        if not in_trade:
            # ── Buscar sweep activo que contenga la barra i ──
            # Un sweep es "activo" a partir de su se_bar
            for sw_i in range(sweep_idx, len(sweeps)):
                sw = sweeps[sw_i]

                # El sweep aún no ha llegado
                if sw.se_bar > i:
                    break
                ...
                # Save full technical context for Pine Script export
                entry_context = {
                    "ss_bar":      sw.ss_bar,
                    "ss_ts":       str(dates[sw.ss_bar]) if sw.ss_bar < n else "",
                    "ss_price":    round(sw.sweep_level, 5),
                    "se_bar":      sw.se_bar,
                    "se_ts":       str(dates[sw.se_bar]) if sw.se_bar < n else "",
                    "se_price":    round(sw.sweep_level, 5),
                    "ol_price":    round(sw.ol_price, 5),
                    "ol_bar":      sw.ol_bar,
                    "ol_ts":       str(dates[sw.ol_bar]) if sw.ol_bar < n else "",
                    "div_bar":     div_bar,
                    "div_ts":      str(dates[div_bar]) if div_bar < n else "",
                    "div_model":   div_model,
                    "bos_bar":     i,
                    "bos_ts":      str(dates[i]),
                    "bos_price":   round(price, 5),
                    "sweep_dir":   sw.direction,
                }
                break
            ...
            # ── Stop Loss hit ──
            if entry_dir == 'bull' and float(data["Low"].values[i]) <= entry_sl:
                exit_price = entry_sl
                exit_type  = "sl"
            elif entry_dir == 'bear' and float(data["High"].values[i]) >= entry_sl:
                exit_price = entry_sl
                exit_type  = "sl"
            else:
                # ── MCT Exit: calcular TP ──
                sl_dist = abs(entry_price - entry_sl)
                tp_dist = abs(entry_ol - entry_price)
                if sl_dist > 0 and tp_dist / sl_dist >= rr_threshold:
                    if entry_dir == 'bull':
                        bb = entry_sl
                        tp = entry_ol - 0.71 * (entry_ol - bb)
                    else:
                        aa = entry_sl
                        tp = entry_ol + 0.71 * (aa - entry_ol)
                else:
                    tp = entry_ol

                # TP hit?
                tp_hit = False
                if entry_dir == 'bull' and float(data["High"].values[i]) >= tp:
                    tp_hit = True
                elif entry_dir == 'bear' and float(data["Low"].values[i]) <= tp:
                    tp_hit = True

                # Cut Early?
                cut_active = False
                if has_cut_early and _CUT_EARLY_AVAILABLE and not tp_hit:
                    try:
                        cut_params = {
                            'direction':   entry_dir,
                        ...
                        cut_signal = _compute_cut_early_ext(data, cut_params)
                        cut_active = bool(cut_signal.iloc[i] == 1.0)
                    except Exception:
                        cut_active = False
                ...
                if tp_hit:
                    ...
                    # Sin señal de salida — mantener trade
            ...
            # ── Cerrar trade ──
            exit_signals += 1
            exec_price = float(exit_price) * (1 - slippage) if entry_dir == 'bull' else float(exit_price) * (1 + slippage)
            ...
            trades.append({
                ...
                "sl_price":    round(entry_sl, 5),
                "tp_price":    round(float(exit_price), 5),
                "ol_price":    round(entry_ol, 5),
...
# Forex pairs that should use Twelve Data instead of yfinance
_FOREX_PAIRS = {
...
def _is_forex(ticker: str) -> bool:
    clean = ticker.upper().replace("=X", "").replace("/", "").replace("-", "")
    return clean in _FOREX_PAIRS


def _download_twelvedata(ticker: str, timeframe: str, start: str, end: str) -> pd.DataFrame:
    """Download OHLCV from Twelve Data API."""
    import os
    import requests
    ...
    if not api_key:
        raise ValueError(
            ...
            "Añádela en Render → Environment → TWELVEDATA_API_KEY"
        )

    # Clean ticker: EURUSD=X → EUR/USD
    clean = ticker.upper().replace("=X", "").replace("-", "")
    if len(clean) == 6:
        symbol = f"{clean[:3]}/{clean[3:]}"
    else:
        symbol = clean

    tf = _TWELVEDATA_TF.get(timeframe, timeframe)

    url = "https://api.twelvedata.com/time_series"
    all_values = []
    current_end = end

    # Paginar hasta cubrir todo el rango (max 5 páginas = 25.000 barras)
    for _ in range(5):
        params = {
            "symbol":     symbol,
            "interval":   tf,
            ...
            "outputsize": 5000,
            "order":      "DESC",  # más reciente primero para paginar hacia atrás
            "apikey":     api_key,
        }
        ...
        # Si devuelve menos de 5000 barras, ya tenemos todo
        if len(values) < 5000:
            break

        # Avanzar el end_date hacia atrás para la siguiente página
    ...
    if not values:
        raise ValueError(
            f"Twelve Data no devolvió datos para '{symbol}' en {tf} "
            f"entre {start} y {end}."
        )
...
def download_data(market: dict) -> pd.DataFrame:
    ...
    # Use Twelve Data for Forex pairs
    if _is_forex(ticker):
        raw = _download_twelvedata(ticker, timeframe, start, end)
    else:
...
INDICATOR_CATALOG = {
    ...
    "cut_early": {
        "name":        "Cut Early Protocol (MCT)",
        "params":      [],
        "conditions":  ["is_true"],
        "needs_value": False,
        "note":        "Salida anticipada por MSB Against + pullback. Estados: IDLE → MSB_PENDING → CUT_ACTIVE.",
    },
}
